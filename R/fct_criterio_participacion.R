...
criterio_participacion_pri <- function(DB_ESTADO, DB_VISITAS){
  ...
  DB_AUX <- select(DB_AUX, c(MUNICIPIO, TOTAL_R_VOTOS, FREC_R_VIS )) %>%
    mutate(CRITERIOP = (FREC_R_VIS - TOTAL_R_VOTOS)^2)
  
  # # # Arrange CP visitas
  
  DB_AUX <- DB_AUX %>% arrange(desc(CRITERIOP))
  DB_ORDENADA <- select(DB_AUX, c(MUNICIPIO, TOTAL_R_VOTOS, CRITERIOP))
  
  return(DB_AUX)
}

S <- criterio_participacion_pri(DB_Mich, fake_visitas)
